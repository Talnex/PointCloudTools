...
{
    printf("%s 的 %s操作 用时 %lf秒\n", pszContainerName, pszOperator, lElapsetime*1.0/CLOCKS_PER_SEC);
}

void deletpoint(pcl::PointCloud<pcl::PointXYZRGB>::Ptr sharedPtr, planenum planenum);

double countOFF(planenum &p1, planenum p2);
...
void nomalize(planenum &p) {
    double m = sqrt(pow(p.A, 2) + pow(p.B, 2) + pow(p.C, 2));
    p.A = p.A / m;
    p.B = p.B / m;
    p.C = p.C / m;
    p.D = p.D / m;
...
double countSD(planenum p1, planenum p2) {
    double n1 = p1.A / p2.A;
    double n2 = p1.B / p2.B;
    double n3 = p1.C / p2.C;
    double mean = (n1 + n2 + n3) / 3;
    return sqrt((pow(n1 - mean, 2) + pow(n2 - mean, 2) + pow(n3 - mean, 2)) / 2);
}

void checksame(vector<planenum> &vec) {
    for (int i = 0; i < vec.size(); ++i) {
        for (int j = i + 1; j < vec.size();) {
            double sd = countSD(vec[i], vec[j]);
            //法向量差异大认为是不同平面
            if (sd > GATE_SD) {
                //cerr << vec[i].num << "\t" << vec[i].A << "\t" << vec[i].B << "\t" << vec[i].C
                     //<< "\t" << vec[i].D << endl;
                //cerr << vec[j].num << "\t" << vec[j].A << "\t" << vec[j].B << "\t" << vec[j].C << "\t"
                     //<< vec[j].D << endl;
                //cerr << "SD:" << sd << endl;
                //cerr << "\nnew\n" << endl;
                j++;
            } else {
                vec.erase(begin(vec) + j);
//                if (countOFF(vec[i], vec[j]) > 1) {
//                    j++;
//                } else {
//                    vec.erase(begin(vec) + j);
//                }
...
//        }
//        if (found) {
//            //TODO:查看是否有相似平面，如果没有则增加
//            if (res_vec.size() == 0) {
//                res_vec.push_back(best);
//            } else {
//                bool issame = false;
//                for (int i = 0; i < res_vec.size(); ++i) {
//                    double sd = countSD(res_vec[i], best);
//                    //cerr << best.num << "\t" << best.A << "\t" << best.B << "\t" << best.C << "\t" << best.D << endl;
//                    //cerr << res_vec[i].num << "\t" << res_vec[i].A << "\t" << res_vec[i].B << "\t" << res_vec[i].C
//                         //<< "\t" << res_vec[i].D << endl;
//                    //cerr << "SD:" << sd << endl;
//                    if (sd > 1) {
//                        issame = false;
//                        //cerr << "\nnew\n" << endl;
//                    } else {
//                        if (countOFF(res_vec[i], best) > 0.5) {
//                            issame = false;
//                            //cerr << "\nparallell\n" << endl;
//                        } else {
//                            issame = true;
...
//        coeffs.values[0] = (res_vec[i].A);
//        coeffs.values[1] = (res_vec[i].B);
//        coeffs.values[2] = (res_vec[i].C);
//        coeffs.values[3] = (res_vec[i].D);
//        viewer->addPlane(coeffs, to_string(res_vec[i].A));
//    }
//
//    pcl::ModelCoefficients coeffs;
//    coeffs.values.resize(4);
...
double countOFF(planenum &p1, planenum p2) {
    ...
    //cerr << "before:" << p1.A << " " << p1.B << " " << p1.C << endl;
    //cerr << "before:" << p2.A << " " << p2.B << " " << p2.C << endl;
    p1.A = p1.A / mean;
    p1.B = p1.B / mean;
    p1.C = p1.C / mean;
    p1.D = p1.D / mean;
    //cerr << "after:" << p1.A << " " << p1.B << " " << p1.C << endl;
    //cerr << "after:" << p2.A << " " << p2.B << " " << p2.C << endl;

    double m1 = (p1.A + p2.A) / 2;
    double m2 = (p1.B + p2.B) / 2;
    double m3 = (p1.C + p2.C) / 2;

    double off = abs(p1.D - p2.D) / sqrt(pow(m1, 2) + pow(m2, 2) + pow(m3, 2));
    //cerr << "off:" << off << endl;
    return off;
}
