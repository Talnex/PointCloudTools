...
/*******************************************************************************************************
 这里是处理鼠标事件的函数，每次相应鼠标事件都会回调函数，需要从event实例提取事件信息，本例中查找鼠标左键的释放事件
...
int main(int argc, char **argv) {
    ...
    // 如果输入的是-h的话就打印帮助信息
    //find_argument:argc,argv,argument_name
    //return 找到返回下标，找不到返回-1
    if (pcl::console::find_argument(argc, argv, "-h") >= 0) {
        printUsage(argv[0]);
        return 0;
    }
    // 这里定义了一堆标志位，根据不同的命令参数设置不同的标志位
    ...
    // 制作一个沿z轴拉伸的椭圆。 颜色将逐渐从红色变为绿色再变为蓝色。
    uint8_t r(255), g(15), b(15);
    for (float z(-1.0); z <= 1.0; z += 0.05) {
        for (float angle(0.0); angle <= 360.0; angle += 5.0) {
            pcl::PointXYZ basic_point;//一个普通的点，有xyz坐标的属性
            basic_point.x = 0.5 * cosf(pcl::deg2rad(angle));
            basic_point.y = sinf(pcl::deg2rad(angle));
            basic_point.z = z;
            basic_cloud_ptr->points.push_back(basic_point);

            pcl::PointXYZRGB point;//一个有颜色的点，有xyz和rgb属性
            point.x = basic_point.x;
            point.y = basic_point.y;
            point.z = basic_point.z;
            //static_cast 相当于强制类型转换
            //这里将3个8位的rgb值拼成一个32位的rgb数据，从高位到低位分别为ARGB，这里A默认全0为不透明
            uint32_t rgb = (static_cast<uint32_t>(r) << 16 |
                            static_cast<uint32_t>(g) << 8 | static_cast<uint32_t>(b));
            //reinterpret_cast 相当于重新解释比特位
            //这里将拼接好的rgb数据转换成了一个float指针
    ...
    pcl::NormalEstimation<pcl::PointXYZRGB, pcl::Normal> ne;//设置输入和输出类型

    ne.setInputCloud(point_cloud_ptr);//设置输入的点云

    pcl::search::KdTree<pcl::PointXYZRGB>::Ptr tree(new pcl::search::KdTree<pcl::PointXYZRGB>());
    ne.setSearchMethod(tree);//使用Kd树搜索

    pcl::PointCloud<pcl::Normal>::Ptr cloud_normals1(new pcl::PointCloud<pcl::Normal>);
    ne.setRadiusSearch(0.05);//设置搜索半径
